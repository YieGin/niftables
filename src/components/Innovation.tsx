      ...
      <div className="grid w-full relative overflow-scroll xl:overflow-hidden hide-scrollbar">
        <div className="flex xl:flex-col z-10 relative xl:overflow-hidden hide-scrollbar md:gap-y-5 px-5 md:px-20 2xl:px-[219px] gap-5 md:gap-x-10">
          {InnovationList.map((item, index) => (
            <div
              key={index}
              className="flex xl:flex-row flex-col-reverse xl:justify-between rounded-md bg-[#13171D] bg-opacity-60 h-full md:w-[400px] sm:w-[80vw] w-[80vw] xl:w-full"
            >
              <div className="xl:w-[70%] p-5 pt-7 xl:p-8 mb-auto">
                <h1 className="xl:text-[38px] leading-[120%] md:text-[20px] text-sm sm:text-[18px] font-monument uppercase xl:min-h-max md:min-h-[4.5rem] min-h-[2.5rem]">
                  {item.title}
                </h1>
                <p className="font-satoshi-Regular md:text-[15px] sm:text-[15px] text-sm xl:text-[18px] leading-[130%] text-white xl:mr-10 mt-2">
                  {item.description}
                </p>
              </div>
              <div className="relative xl:w-[480px] rounded-md z-20">
                <div className="video-overlay-Innovation-video-bottom rounded-l-md rounded-r-md xl:rounded-r-none opacity-70" />
                <Image
                  width={1000}
                  height={1000}
                  className="object-cover w-full h-full rounded-r-md rounded-l-md xl:rounded-l-none z-20"
                  src={item.image}
                  alt={item.title}
                />
              </div>
