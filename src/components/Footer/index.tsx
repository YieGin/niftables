...
const Footer = () => {
  return (
    <footer className="px-5 md:px-20 2xl:px-[219px] pb-9 flex md:justify-between items-center flex-col md:flex-row font-satoshi-Regular bg-opacity-5 z-10 relative">
      <div className="flex items-center flex-col md:flex-row gap-x-10 lg:gap-x-[90px]">
        ...
        <div className="flex gap-[10px]">
          {SocialLinks.map((item, index) => (
            <Link className="hover:bg-[#3d8bff] cursor-pointer delay-75 rounded-full my-5 md:my-0" target="_blank" href={item.link} key={index}>
            <Image
              className="h-max"
              alt="Logo"
              src={item.icon}
      ...
      <div className="flex gap-[6px] items-center group cursor-pointer">
